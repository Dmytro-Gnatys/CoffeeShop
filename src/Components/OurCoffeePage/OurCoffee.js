...
const OurCoffee = ({ defsultValues }) => {
  ...
  const onUpdateCoffeeItems = useCallback(
    (country) => {
      if (["Columbia", "Kenya", "Brazil"].includes(country)) {
        const filteredItems = defsultValues.filter(
  ...
  const visibleData = filterPost(serchEmp(coffeeItem, term), filterSearch);
      ...
      <div className="content">
        <div className="ourbeens_form_wrapper">
          ...
          <AppFilter
            ...
            onUpdateCoffeeItems={onUpdateCoffeeItems}
          />
        </div>
